---
theme: "white"
transition: "slide"
title: "Ask me Anything"
enableMenu: true
enableSearch: false
enableChalkboard: true
slideNumber: true
hashOneBasedIndex: false
---

### Luís Antônio (Tonhão)

Software Engineer at Techbiz Forense Digital

Egresso do IFAC (Técnico em Informática)

Prof. na Algorithms School

---

### Júlio Rodrigues

Software Engineer at Magalu

Egresso do IFAC (Técnico em Informática)

---

### Sobre a apresentação

- Nossa trajetória no IFAC  
- Como entramos no mundo da programação  
- Experiências profissionais  
- **Pergunte qualquer coisa (AMA)**

---

### Antes do IFAC

---

### Nossa jornada no IFAC

- 2016–2019 → Técnico em informática (4 anos)
- Participamos de projetos, maratonas e eventos  
- Conheci áreas diferentes: redes, bancos de dados, web, segurança, design e etc. 

---

### Projeto marcante: Sistema de fichas de almoço

Nosso primeiro projeto "real":  
- Parceria com outro colega: Anderson (leigo)
- Feito para resolver um problema interno  
- Desenvolvido com HTML, CSS, Javascript, PHP e Flutter
- Nos ensinou:  
  - versionamento com Git  
  - trabalhar com usuários reais  
  - importância de UX e simplicidade

---

### Transição para o curso superior   

---

### Como entrei no mercado?

QI (Quem indica)

---

### Carreira até hoje (Tonhão)

**2020 → Recol**  
- Dashboards e sistemas internos  
- Crescimento rápido em React

---

### Carreira até hoje (Tonhão)

**2021–2023 → Usabit Global**  
- Mobile + Web  
- Liderança do primeiro produto da empresa  
- Terceirizada de desenvolvedores, sede no Rio de Janeiro (Capital)

---

### Carreira até hoje (Tonhão)

**2023–2024 → MB Labs**  
- App de financiamento de campanhas coletivas  
- Cinemark, rede de cinemas enorme na America Latina
- Terceirizada de desenvolvedores, sede em Campinas - São Paulo

---

### Carreira até hoje (Tonhão)

**2024–2024 → B&T Câmbio**  
- Sustentação dos sistemas da maior corretora de câmbio do Brasil  
- Implementação de métricas e observabilidade
- Sede em São Paulo (Capital), na avenida Paulista

---

### Carreira até hoje (Tonhão)

**2025–Atual → Techbiz Forense**  
- Snap Reports (OSINT)  
- Clientes: MPs, Polícia Civil, Polícia Científica, Polícia Federal etc.
- Sede em Belo Horizonte - Minas Gerais

---

# Júlio

---

### Carreira até hoje (Júlio)

**2019-2021 → Estágio Prefeitura** 
- Manutenção de computadores
- Suporte ao usuário
- Desenvolvimento

---

### Carreira até hoje (Júlio)

**2021-2022 → Desenvolvedor Prefeitura** 
- Manutenção de sistemas legados
- Desenvolvimento de novos sistemas

---

### Carreira até hoje (Júlio)

**2021-2022 → Desenvolvedor SEPLAG**

---

### Carreira até hoje (Júlio)

**2023-2024 → Desenvolvedor Backend TJAC**

---

### Carreira até hoje (Júlio)

**2024-atual → Desenvolvedor Backend Magalu**

---

### O que realmente mudou nossa carreira

- Consistência (todo dia um pouco)
- Leitura (livros!)
- Inglês (ao menos para consumir conteúdo)
- Entender **A BASE** (JS, HTTP, Algoritmos)  
- Não ter medo de errar
- Não ser **TÍMIDO!** (!= introvertido) 
- Comunidade

---

### Algorithms School

- Iniciativa para formar devs no Acre  
- Foco em programação competitiva  
- Três edições, +100 alunos  
- Torneio de programação no Shopping!
- Objetivo: criar comunidade e oportunidade no estado  

---

### Dicas para quem está começando

- Aprenda os fundamentos **DE VERDADE**  
- Comece simples: pequenos projetos  
- Suba tudo no GitHub  
- Contribua em comunidade  
- Se compare **com você ontem**, não com dev sênior no LinkedIn

---

### Oportunidades no IFAC

- Maratonas de programação
- Projetos de pesquisa e extensão  
- Professores acessíveis  
- Laboratório e comunidade  
- Networking que dura anos

---

### Lições que o IFAC me deixou

- Você não precisa ser gênio  
- A curva de aprendizado é longa
- Peça ajuda
- O ambiente pode transformar carreiras  
- As pessoas que você conhece importam

---

### Obrigado!

Tonhão: [https://www.linkedin.com/in/tonhao-dev/](https://www.linkedin.com/in/tonhao-dev/)  
___

Júlio: [https://www.linkedin.com/in/juliocesar-dev/](https://www.linkedin.com/in/juliocesar-dev/)

---

### Ask Me Anything 🙋‍♂️

Sugestões de perguntas:

- Como conseguir primeiro emprego?  
- Portfólio ideal?  
- O que estudar em 2025?  
- Vale a pena sair do Acre?  
- Quanto ganha um dev frontend?  
- Mercado de trabalho hoje?
